class WebhooksController < ApplicationController
  def new
    @webhook = Webhook.new
    @card = session[:card]['pan'].gsub(/(.{4})/, '\1 ')
  ...
  def create
    ...
    alert_message = "Your Marqeta account has just received an authorization request for $#{params['webhook'][:amount]}0 from #{params['webhook'][:user]} at #{params['webhook'][:merchant]} located in #{params['webhook'][:city]},#{params['webhook'][:state]}, processed over the #{params['webhook'][:network]} network. It is currently in #{params['webhook'][:status]} state."
